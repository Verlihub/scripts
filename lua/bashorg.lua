...
conf = {
	...
	tran = {
		...
		ru = {											-- russian translation
			quot = "Ñëó÷àéíàÿ öèòàòà ñ áàøîðã.ðô:",
...
function bash ()
	...
	if os.execute ("curl --get --location --max-redirs 1 --retry 2 --connect-timeout 5 --max-time 10 --user-agent \"Mozilla/5.0 (compatible; BashOrg/0.5.3.8; +https://ledo.feardc.net/other/)\" --silent --output \"" .. conf.path .. "/" .. conf.file .. ".curl\" \"https://xn--80abh7bk0c.xn--p1ai/random\"") then
			...
			if os.execute ("iconv -s -c -f \"UTF-8\" -t \"CP1251\" -o \"" .. conf.path .. "/" .. conf.file .. ".conv\" \"" .. conf.path .. "/" .. conf.file .. ".curl\"") then
				...
				if file then
					...
					if data and # data > 0 then
						local quot = data:match ("<div class=\"quote__body\">(.-)</div>")

						if quot and # quot > 0 then -- and not quot:find (string.char (208, 160, 208, 133, 208, 160), 1, true)
							quot = quot:gsub ("&lt;", "<")
							quot = quot:gsub ("&gt;", ">")
							quot = quot:gsub ("&#60;", "<")
							quot = quot:gsub ("&#62;", ">")
							...
							quot = quot:gsub ("&amp;", "&")
							quot = quot:gsub ("<br.->", "\r\n")
							quot = quot:gsub ("<div class=\"quote__strips\"[^>]*>[\r\n ]*<h3 class=\"quote__strips_title\">[\r\n ]*(.-)[\r\n ]*</h3>[\r\n ]*<ul class=\"quote__strips_list\">[\r\n ]*<li class=\"quote__strips_item\">[\r\n ]*<a href=\"/strip/%d+\" class=\"quote__strips_link\">[\r\n ]*<img src=\"([^\"]+)\" class=\"quote__strips_img\">[\r\n ]*</a>[\r\n ]*</li>[\r\n ]*</ul>[\r\n ]*", "%1: https://áàøîðã.ðô%2")
							quot = quot:gsub (" +", " ")
							quot = quot:gsub ("^[\r\n ]+", "")
							quot = quot:gsub ("[\r\n ]+$", "")
							quot = quot:gsub (" *\r\n *", "\r\n ")
