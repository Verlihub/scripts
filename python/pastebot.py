...
pb_defs = {
	...
	"content": "content",
	"text": "text",
	"agent": "PasteBot/0.0.2.4", # todo: dont forget to update
	"timeout": 5,
	"command": "paste",
	"shortcut": "p",
	"class": 3,
...
def pb_notify (nick, data, oc):
	if oc:
		...
		vh.ScriptCommand ("opchat_to_all", "<%s> %s" % (nick, data)) # to catch in ledokol
	else:
		vh.mc (pb_nmdc (data), nick)
		vh.ScriptCommand ("chat_to_all", "<%s> %s" % (nick, data)) # to catch in ledokol
